        ...
        requireView().post {
            checkClipboard().apply {
                this?.apply {
                    ...
                    // ── Pre-fetch ──────────────────────────────────────────────────────
                    // Start extracting video metadata immediately in the background so the
                    // download sheet can open the instant the user taps the FAB instead of
                    // making them wait 3-5 seconds for yt-dlp / NewPipe to run.
                    if (this.size == 1) resultViewModel.startPrefetch(this.first())
                    // ──────────────────────────────────────────────────────────────────
                    ...
                    clipboardFab?.setOnClickListener {
                        if (this.size == 1){
                            ...
                            lifecycleScope.launch {
                                // Await the pre-fetch (may already be done → returns instantly)
                                val prefetched = withContext(Dispatchers.IO) {
                                    resultViewModel.awaitPrefetchResult(url)
                                }
                                if (prefetched != null) {
                                    // Data ready — open sheet immediately, no spinner
                                    showSingleDownloadSheet(
                                        resultItem = prefetched,
                                        type = DownloadType.valueOf(
                                            sharedPreferences!!.getString("preferred_download_type", "video")!!
                                        )
                                    )
                                } else {
                                    // Pre-fetch failed or timed out — fall back to normal flow
